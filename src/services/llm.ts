...
export class LLMService {
  ...
  async chat(
    ...
    try {
      const result = currentModel === 'claude'
        ? await this.anthropicChat(effectiveEndpoint, apiKey, effectiveModel, messages, temperature, maxTokens, onChunk)
      ...
      if (!cleaned.trim()) {
        const error = new Error('接口返回空内容，请检查模型、API Endpoint 或 API Key 是否匹配')
        logger.error('LLM 返回空内容', { provider: currentModel, model: effectiveModel })
        throw error
      }
      this.markDiagnosticsSuccess(cleaned)
      logger.info('LLM 请求成功', {
        provider: currentModel,
        latencyMs: llmDiagnostics.latencyMs,
        responseLength: cleaned.length
      })
      return cleaned
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error || '请求失败')
      logger.error('LLM 请求失败', {
        provider: currentModel,
        error: message,
        endpoint: effectiveEndpoint
      })
  ...
  private async openAICompatibleChat(
    ...
    if (provider === 'minimax' || provider === 'minimax_global') {
      ...
      if (maxTokens > 2048) {
        console.warn(`[MiniMax] maxTokens ${maxTokens} 超过支持上限 2048，已自动截断`)
      }
    } else {
    ...
    if (onChunk && isTauri()) {
      body.stream = false
      const result = await this.normalOpenAICompatible(endpoint, apiKey, body)
      onChunk(result)
      return result
    }
    ...
    if (onChunk) {
      try {
        const streamed = await this.streamOpenAICompatible(endpoint, apiKey, body, onChunk)
    ...
    return await this.normalOpenAICompatible(endpoint, apiKey, body)
  }

  private async normalOpenAICompatible(endpoint: string, apiKey: string, body: Record<string, unknown>): Promise<string> {
    return await this.withTimeout(async (signal) => {
      const headers = {
        'Content-Type': 'application/json',
        ...(apiKey ? { Authorization: `Bearer ${apiKey}` } : {})
      }
      const response = await this.postJson(endpoint, headers, JSON.stringify(body), signal)
  ...
  private async anthropicChat(
    ...
    if (onChunk && isTauri()) {
      // Tauri 流式模式：先走非流式版本获取完整内容，再通过 onChunk 推送
      const result = await this.anthropicChat(endpoint, apiKey, model, messages, temperature, maxTokens)
      onChunk(result)
      return result
    }
  ...
  private async withTimeout<T>(operation: (signal: AbortSignal) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      if (
        ...
        throw new Error(`请求超时：${REQUEST_TIMEOUT_MS / 1000} 秒内没有收到模型回复，请检查网络、代理或 Endpoint`)
      }
      throw error
    } finally {
  ...
  private extractContent(data: OpenAICompatibleResponse): string {
    ...
    for (const candidate of candidates) {
      ...
      if (Array.isArray(candidate)) {
        const text = candidate.map((item) => item.text || item.content || '').join('')
    ...
    if (Array.isArray(outputContent)) {
      return outputContent.map((item) => item.text || '').join('')
    }

    const candidateContent = data.candidates?.[0]?.content?.parts
    if (Array.isArray(candidateContent)) {
      return candidateContent.map((item) => item.text || '').join('')
    }
