...
export async function saveSecret(account: string, value: string): Promise<void> {
  const saved = await tryInvoke<void>('save_api_key', { account, apiKey: value })
  if (saved === null) {
    sessionStorage.setItem(`api_key:${account}`, value)
  }
}
...
export async function loadSecret(account: string): Promise<string> {
  ...
  return value ?? sessionStorage.getItem(`api_key:${account}`) ?? ''
}
