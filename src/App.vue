...
const cacheText = computed(() => {
  if (!sourceCacheUpdatedAt.value) return `未刷新，本次构建 ${buildTime}`
...
const normalizeSettings = () => {
  ...
  if (settingsStore.temperature < 0.1) settingsStore.temperature = 0.1
  if (settingsStore.maxTokens > 2048) settingsStore.maxTokens = 2048
}
